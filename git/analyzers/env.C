...
void env(int type){
	int inp;
	TFile* f = new TFile("../diphox_completed.root");
	TFile test("test.root","recreate");
	TH1D nom;
	TH1D* h;
	TH1D upperThres("up","Upper Excursion",172,0.0,8600.0); 
	TH1D lowerThres("down","Lower Excursion",172,0.0,8600.0);
	...
	if (type == 0){
		...
		for (int i = 0; i < 172; i++){
			...
			for (int j = 0; j < 52; j++){
				...
				if (fabs(nom.GetBinContent(i)-h->GetBinContent(i))>move) move=fabs(nom.GetBinContent(i)-h->GetBinContent(i));
			}
			reportDouble(move);
