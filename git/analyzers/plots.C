...
void plots(int ignvctr){
	...
	c1->cd(); stack1.Draw("nostack"); c1->BuildLegend(); c2->cd(); stack2.Draw("nostack"); c2->BuildLegend(); c3->cd(); stack3.Draw("nostack"); c3->BuildLegend(); //c4->cd(); stack4.Draw(); c4->BuildLegend();
	...
	stream.str(""); stream<<"EEset"<<ignvctr<<".pdf"; str = stream.str(); c1->SaveAs(str.Data());
	stream.str(""); stream<<"BBset"<<ignvctr<<".pdf"; str = stream.str(); c2->SaveAs(str.Data());
	stream.str(""); stream<<"BEset"<<ignvctr<<".pdf"; str = stream.str(); c3->SaveAs(str.Data());
